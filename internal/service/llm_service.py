...
class LLMService:
    ...
    def _build_prompt(self, question: str, context: str,
        ...
        if allow_full_doc_retrieval:
            full_doc_hint = """
5. 【整篇文档召回标记】如果你判断当前"已知信息"碎片化、缺少全局背景、或无法覆盖用户问题的全貌，请输出一个或多个标记请求完整文档：
   {{retrieve_full_doc:文件名}}
   - 每个文件只能标记一次，标记之间用换行分隔
   - 标记必须独立成行（前后无其他字符）
   - 同时给出你认为需要的补充检索方向（先用一行简要说明）
   - 系统会召回完整文档并自动重新生成答案；你不需要解释标记的含义
6. 【关键禁止】你的最终答案中**绝对不能**包含任何 `{{retrieve_full_doc:...}}` 标记 —— 标记仅用于触发检索，不可直接出现在面向用户的回答中。如果你判断不需要额外文档，则**不要**输出任何标记。"""
...
2. 如果"已知信息"完全不相关，请如实说明"知识库中未找到相关内容"，并提示用户换个问法或上传相关文档。
